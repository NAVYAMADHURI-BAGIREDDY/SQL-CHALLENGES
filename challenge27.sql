...
INSERT INTO walmart_eu27(invoiceno, stockcode, description, quantity, invoicedate, unitprice, customerid, country) VALUES
...
FROM (
    SELECT 
        ...
        description,
        total,
        DENSE_RANK() OVER (PARTITION BY months ORDER BY total DESC) AS rnk
    FROM 
        ranking
) AS ranked
WHERE 
...
select * from 
(select *,
dense_rank() over(partition by customerid order by quantity desc) 
as rnk from walmart_eu27) as ranking 
where rnk=1;
