use sqlchallenge;
CREATE TABLE amazon_products39(
...
INSERT INTO amazon_products39(product_name, category, price, country) VALUES
...
INSERT INTO return_records39(order_id, product_id, return_reason, return_date) VALUES
...
for each category.
*/
-- /(select count(*) from return_records39) * 100 as percentage_returns
select a.category,count(price)           
from amazon_products39 a
join return_records39  r
on a.product_id = r.product_id
group by category; 

SELECT
    ap.category,
    COUNT(rr.price)/(SELECT COUNT(*) 
                 FROM return_records)* 100
    AS percetage_return
    
FROM amazon_products39 as ap
JOIN return_records39 as rr
