...
INSERT INTO sales_data31(seller_id, total_sales, product_category, market_place, month)
VALUES
...
*/
select  * from sales_data31;
select  product_category,
    seller_id,
    sales from
(select product_category,sum(total_sales) as sales,
        seller_id,dense_rank() over(partition by product_category order by sum(total_sales) desc) as rnk
from sales_data31
where extract(month from month)= 1 
group by product_category,seller_id) as sub
where rnk<=3;
