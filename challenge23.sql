...
WITH RankedPurchases AS (
    SELECT 
        user_id,
        purchase_date,
       DENSE_RANK() OVER (PARTITION BY user_id ORDER BY purchase_date) AS rnk,
...
SELECT 
    user_id,
    previous_purchase_date AS first_purchase_date,
...
WHERE 
    rnk = 2 and
    DATEDIFF(purchase_date, previous_purchase_date) < 7;
