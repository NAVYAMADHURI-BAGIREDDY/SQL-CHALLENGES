...
INSERT INTO bookings27(id, hotel_name, booking_date, cust_id, adult, payment_type) VALUES
...
select * from bookings27;
SELECT hotel_name, 
sum(CASE 
   when  DAYOFWEEK(booking_date) in (6,7) then 1
   else 0
end) as counting
FROM bookings27
group by hotel_name;
...
SELECT 
    hotel_name,
    SUM(CASE 
            WHEN DAYOFWEEK(booking_date) IN (6, 7)
            THEN 1
            ELSE 0
